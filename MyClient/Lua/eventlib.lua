...
function _M:Connect(v_fnHandler,v_oHost)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    assert(type(v_fnHandler) == "function" , "Invalid handler. Expected function got " .. type(v_fnHandler))
    assert(self.handlers, "Invalid Event")
    if v_oHost then
        table.insert(self.handlers, {fn = v_fnHandler,host = v_oHost})
    else
        table.insert(self.handlers, v_fnHandler)
    end
    local t = { }
    t.Disconnect = function()
        return self:Disconnect(v_fnHandler)
    end
